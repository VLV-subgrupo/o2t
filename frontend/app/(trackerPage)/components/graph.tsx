...
const CustomTooltip = ({ active, payload, label } : TooltipProps<number,string>) => {
    if (active && payload && payload.length) {
        ...
        return (
            <div className="flex flex-row border border-gray bg-darkgray p-2 rounded-lg w-48 justify-between items-center">
                <p className="label text-lightgray">weight</p>
                <div className="flex flex-row items-center gap-1">
                    <p className="text-light">
                        {` ${payload[1].value}`}
                    </p>
                    {!show && <p className="text-light ml-2 label px-2 rounded-sm bg-gray">
                        not registered
                    </p>}
                    {show &&
...
const Graph = () => {
    const data = [
        ...
        {date: "08.06", value: 70, goal: 75},
        {date: "09.06", value: 71, goal: 75},
    ...
    return (
        <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} className=" select-none" margin={{top: 60}}>
                <XAxis dataKey="date" className="p font-semibold last:text-light" padding={{ left: 48, right: 32 }} strokeOpacity={0}/>
                <YAxis padding={{bottom: 10 }} className="p font-semibold text-light" domain={[0, (dataMax : number) => Math.round(dataMax * 1.5 / 10) * 10 ]} strokeOpacity={0}>
                </YAxis>
                <Tooltip cursor={false} animationEasing="ease-out" content={CustomTooltip} />
